...
class HIDCHOP final : public CHOP_CPlusPlusBase {
public:
    HIDCHOP(const OP_NodeInfo*) {}
    ~HIDCHOP() override { stop(); }
...
DLLEXPORT void FillCHOPPluginInfo(CHOP_PluginInfo* i) {
    ...
    i->customOPInfo.opType->setString("Hid");
    i->customOPInfo.opLabel->setString("HID");
    i->customOPInfo.opIcon->setString("HID");
    i->customOPInfo.authorName->setString("TDAppleML");
    i->customOPInfo.minInputs=0; i->customOPInfo.maxInputs=0;
}
DLLEXPORT CHOP_CPlusPlusBase* CreateCHOPInstance(const OP_NodeInfo* i) { return new HIDCHOP(i); }
DLLEXPORT void DestroyCHOPInstance(CHOP_CPlusPlusBase* i) { delete static_cast<HIDCHOP*>(i); }
}
