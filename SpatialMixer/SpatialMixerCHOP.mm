...
class SpatialMixerCHOP final : public CHOP_CPlusPlusBase {
public:
    SpatialMixerCHOP(const OP_NodeInfo*) {}
    ~SpatialMixerCHOP() override { @autoreleasepool { teardown(); } }
...
private:
    ...
    bool ensureEngine(double sr, int maxFrames, int nch, const std::string& layout) {
        ...
        __block SpatialMixerCHOP* self_=this;
        for (int c=0;c<nch;c++) {
            __block int ch=c;
...
DLLEXPORT void FillCHOPPluginInfo(CHOP_PluginInfo* i) {
    ...
    i->customOPInfo.opType->setString("Spatialmixer");
    i->customOPInfo.opLabel->setString("Spatial Mixer");
...
DLLEXPORT CHOP_CPlusPlusBase* CreateCHOPInstance(const OP_NodeInfo* i) { return new SpatialMixerCHOP(i); }
DLLEXPORT void DestroyCHOPInstance(CHOP_CPlusPlusBase* i) { delete static_cast<SpatialMixerCHOP*>(i); }
}
