...
namespace {
class SpatialAudioCHOP final : public CHOP_CPlusPlusBase {
public:
    SpatialAudioCHOP(const OP_NodeInfo*) {}
    ~SpatialAudioCHOP() override { @autoreleasepool { teardown(); } }
...
private:
    ...
    bool ensureEngine(double sr, int maxFrames) {
        ...
        myMonoFmt = monoFmt;
        __block SpatialAudioCHOP* self_ = this;
...
DLLEXPORT void FillCHOPPluginInfo(CHOP_PluginInfo* i) {
    ...
    i->customOPInfo.opType->setString("Spatialaudio");
    i->customOPInfo.opLabel->setString("Spatial Audio");
...
DLLEXPORT CHOP_CPlusPlusBase* CreateCHOPInstance(const OP_NodeInfo* i) { return new SpatialAudioCHOP(i); }
DLLEXPORT void DestroyCHOPInstance(CHOP_CPlusPlusBase* i) { delete static_cast<SpatialAudioCHOP*>(i); }
}
