...
extern "C"{DLLEXPORT void FillCHOPPluginInfo(CHOP_PluginInfo*i){if(!i->setAPIVersion(CHOPCPlusPlusAPIVersion))return;i->customOPInfo.opType->setString("Visionsimilarity");i->customOPInfo.opLabel->setString("Vision Similarity");i->customOPInfo.opIcon->setString("VSM");if(i->customOPInfo.opHelpURL)i->customOPInfo.opHelpURL->setString("https://github.com/sygnalinc/TDAppleOps/blob/main/VisionSimilarity/README.md");i->customOPInfo.authorName->setString("SYGNAL Inc.");
