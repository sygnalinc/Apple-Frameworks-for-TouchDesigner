// RealityKit Splat TOP — 3D Gaussian Splat(.ply)/ USD / USDZ / Reality を RealityRenderer で
// オフスクリーン描画してTOPに出す。macOS 27 の GaussianSplatComponent により .ply を真の
// ガウシアンスプラットとして描画する(3DGSの生バッファを LowLevelBuffer で投入)。
// 描画はSwiftヘルパ(RealityKitSplatHelper)がMainActorで実行。cookは非ブロック。
...
extern "C" {
    ...
    void  rk_set_scene(void*, const char*, int maxSplats);
    void  rk_set_camera(void*, float dist, float yaw, float pitch, float fov, float tx, float ty, float tz);
    void  rk_set_rotate(void*, float rx, float ry, float rz);
...
public:
    ...
    void execute(TOP_Output* out, const OP_Inputs* in, void*) override {
        ...
        rk_set_scene(myState, file.c_str(), (int)in->getParInt("Maxsplats"));
        ...
        double rot[3]; in->getParDouble3("Rotate", rot[0], rot[1], rot[2]);
        rk_set_rotate(myState, (float)rot[0], (float)rot[1], (float)rot[2]);
        ...
        myRender = rk_render(myState, W, H, 1.f/60.f, spin);
    ...
    void setupParameters(OP_ParameterManager* m, void*) override {
        ...
        { OP_StringParameter p("File"); p.label = "Scene File (3DGS .ply / USD / USDZ / Reality)"; p.page = PAGE; m->appendFile(p); }
        { OP_NumericParameter p("Maxsplats"); p.label = "Max Splats (subsample)"; p.page = PAGE; p.defaultValues[0] = 500000;
          p.minSliders[0] = 10000; p.maxSliders[0] = 2000000; p.minValues[0] = 1000; p.clampMins[0] = true; m->appendInt(p); }
        ...
        { OP_NumericParameter p("Autorotate"); p.label = "Auto Rotate (deg/s)"; p.page = PAGE; p.defaultValues[0] = 0;
          p.minSliders[0] = -180; p.maxSliders[0] = 180; m->appendFloat(p); }
        { OP_NumericParameter p("Rotate"); p.label = "Content Rotate (deg)"; p.page = PAGE;
          for (int i = 0; i < 3; i++) { p.defaultValues[i] = 0; p.minSliders[i] = -180; p.maxSliders[i] = 180; } m->appendXYZ(p); }
    ...
    void getInfoCHOPChan(int32_t i, OP_InfoCHOPChan* c, void*) override {
        const char* n[] = {"executes", "frames", "render_rc", "loaded", "splats"};
        std::string js;
        if (myState) { const char* j = rk_status_json(myState); if (j) { js = j; free((void*)j); } }
        int isLoaded = (js.find("\"loaded\":\"\"") == std::string::npos && js.find("loaded") != std::string::npos) ? 1 : 0;
        float v[] = {(float)myExec.load(), (float)myFrames.load(), (float)myRender.load(), (float)isLoaded,
                     (float)(myState ? rk_splat_count(myState) : 0)};
...
DLLEXPORT void FillTOPPluginInfo(TOP_PluginInfo* i) {
    ...
    if (i->customOPInfo.opHelpURL)
        i->customOPInfo.opHelpURL->setString("https://github.com/sygnalinc/TDAppleOps/blob/main/RealityKitSplat/README.md");
