...
namespace {
class BeaconCHOP final : public CHOP_CPlusPlusBase {
public:
    BeaconCHOP(const OP_NodeInfo*) {}
    ~BeaconCHOP() override { stop(); }
...
DLLEXPORT void FillCHOPPluginInfo(CHOP_PluginInfo* i) {
    ...
    i->customOPInfo.opType->setString("Beacon");
    i->customOPInfo.opLabel->setString("Beacon");
...
DLLEXPORT CHOP_CPlusPlusBase* CreateCHOPInstance(const OP_NodeInfo* i) { return new BeaconCHOP(i); }
DLLEXPORT void DestroyCHOPInstance(CHOP_CPlusPlusBase* i) { delete static_cast<BeaconCHOP*>(i); }
}
