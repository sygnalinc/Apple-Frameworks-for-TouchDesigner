...
namespace {
class WiFiMonitorCHOP final : public CHOP_CPlusPlusBase {
public:
    WiFiMonitorCHOP(const OP_NodeInfo*) {}
    ~WiFiMonitorCHOP() override {}
...
DLLEXPORT void FillCHOPPluginInfo(CHOP_PluginInfo* i) {
    ...
    i->customOPInfo.opType->setString("Wifimonitor");
    i->customOPInfo.opLabel->setString("WiFi Monitor");
...
DLLEXPORT CHOP_CPlusPlusBase* CreateCHOPInstance(const OP_NodeInfo* i) { return new WiFiMonitorCHOP(i); }
DLLEXPORT void DestroyCHOPInstance(CHOP_CPlusPlusBase* i) { delete static_cast<WiFiMonitorCHOP*>(i); }
}
