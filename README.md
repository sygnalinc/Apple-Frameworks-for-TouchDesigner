...
| [Cinematic Video](Cinematic/) | TOP | **iPhone Cinematic video** (macOS 26+): depth (disparity) map or **re-render with adjustable focus / aperture**. **Plays automatically like a Movie File In** (Play Mode: Sequential / Locked to Timeline / Specify Index, plus Speed / Loop / Cue). `Mode = All` emits **rendered, original colour and depth as three colour buffers** (`Color + Depth` is the lighter two-buffer variant); metadata (focus depth, subjects) on its **Info CHOP** |
