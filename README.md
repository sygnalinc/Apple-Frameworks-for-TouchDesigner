...
|---|---|---|---|
| **[0.9.7](https://github.com/sygnalinc/Apple-Frameworks-for-TouchDesigner/releases/tag/v0.9.7)** | 2026-08-14 | 65 | **Five operators promoted to the release.** **MapKit MapView / LookAround / Search** — Apple Maps live in a TOP: fly the 3D map at 57fps, Look Around street imagery with two-way view direction, and search / geocode / routes / coverage in a DAT. **CoreMIDI Out / In** — MIDI with hot-plug and stable device identity, DAW transport (MMC) and sync (MIDI Clock / MTC), and an input that builds its own channels as keys / pads / knobs arrive. Fixes a **crash on every TouchDesigner quit** (CoreMIDI stopped an already-freed run loop) and a **stale saved state that sent the MapKit example to the North Pole** on the next launch. The MapKit windows now close themselves when cooking stops and always start closed. |
