...
### 2026-08-19 AudioUnit CHOP: learn できる数を 16 → 128 に

- ユーザー報告「`all learn slots are used` が出る。50くらいは用意しておきたい」
- `kLearnSlots` は **16個の Learn 枠を持っていた頃の名残**で、枠を廃止してパネル(Script CHOP)に
  移した後も上限として残っていた。**128** に引き上げ(初期化はコンストラクタのループへ)。
  警告文も「上限いくつか」と Clear Learned への導線を出すように
- **実測(M2・AUNBandEQ・41パラメータ)**: `Learned Mapping` に41件を入れて
  **パネルに41個のパラメータと41chが生成**され、警告なし。往復も正しい
  (パネル 0.8 → Global Gain の -96〜24dB で **0dB** = 位置0.8 と一致)。cook 0.559ms・60fps
- **ついでに見つかった穴を修正**: **プラグインを切り替えてもパネルが更新されなかった**。
  `restoreLearnMap` は割り当てを復元するがパネルへ渡し直していなかったため、
  前のプラグインのつまみが残ったままになる(実測: Learnmap に41件入っているのに
  パネルは前の14個のまま)。復元時に `myPanelDirty` を立てるようにした
- 旧 .toe 互換: `Learnmap` のパーサは項目数に依存しないので、16件で保存された .toe もそのまま読める

### 2026-08-19 AudioUnit CHOP: Input Range にパネルが追従するように

- ユーザー要望「Input Range を切り替えたら learned parameter の値も自動で切り替わるように」
- `Input Range` を「**入力1に来る数値の意味**」と位置づけ、**生成パネルもその単位で作り直す**
  ようにした。Normalized=0〜1 のつまみ位置 / Raw=実単位(Hz・ms・dB、範囲は min〜max)
- 実装: spec に実値 `v` と単位モード `rw` を載せ、**署名(sig)にモードを含める**ので
  切り替えると必ず作り直される。書き戻し(`pushToPanel`)も単位に合わせる
- **切り替えの過渡で値が飛ばないようにした**: パネルの作り直しは1フレーム遅れるので、
  その間に古い単位の値を op が書くと大きく飛ぶ。**パネルが `__range` チャンネルで
  「今どちらの単位で出しているか」を申告**し、op は印が自分の設定と一致するまで入力を適用しない
- **実測(M2・AUDistortion)**: Normalized→Raw で `Delay_Mix` 0.757→75.7(範囲0〜100)、
...
### 2026-08-19 MIDI ファイル再生は TD 標準でできる(新規 op 不要)

- ユーザー「MIDI ファイルを再生して MIDI データを出す CHOP を作りたいが TD 標準でできないか」
  → **できる。MIDI In CHOP に `MIDI Source = Device / File` がある**(`.mid` はローカルでもURLでも可)
- **出力の形が重要**: ライブのストリームではなく、**ファイル全体を時間サンプル列(クリップ)**として返す。
  実測: 601サンプル / 60fps(= タイムライン範囲 0..600 に対応)。自作の検証ファイル(120BPM・4音)の
  ノートが **0-0.5 / 0.5-1.0 / 1.0-1.5 / 1.5-2.5 秒**にサンプル単位でぴったり一致した
- **`Note Output = separate`** にすると **`ch1n60` 形式**のチャンネルになる = **AU Instrument に
  そのまま繋がる形**。`1 Based Index` は**既定 On で `ch1n61` が note 60**(1ずれる)。Off で生の MIDI 番号
- **「再生」する配線**: `MIDI In CHOP(File)` → `Trim CHOP`(Unit Values=absolute / Start Unit=End Unit=
  samples / Start=End=`me.time.frame`)→ 現在フレームの1サンプル。実測でフレーム74 → `ch1n67` と
  ファイルどおりに進む。ここから AU Instrument や MIDI Out CHOP へ
- MIDI In **DAT** はデバイス専用(ファイル指定は無い)
- **公式ドキュメントには再生タイミングの記載が無い**(パラメータ説明のみ)。クリップとして出ることは実測で確定

#### 踏んだ誤診(2つとも既出の型)

- **`if chan:` はチャンネルの存在ではなく値で真偽が決まる**。`if n.chan('ch1n64')` が False になり
  「チャンネルが無い」と誤読した。`is None` で判定する([[VisionPose3D の root:tx と同じ]])
...
### 2026-08-19 バンド編成(ドラム/ギター/ピアノ/ベース)の MIDI 素材

- ユーザー「ドラムとギターとピアノとベースの音色が入ったサンプルはない?」
- **配布できるバンド編成の MIDI は現実的に存在しない**。バンド物の MIDI はほぼポップスのカバーで、
  ①楽曲自体が著作権あり ②MIDI データ側のライセンスも不明、の二重で使えない。
  → **曲がパブリックドメインのものを自分で編曲する**のが唯一クリーンな道
- `Assets/sample_band.mid` を自作(1898バイト・116BPM・8小節・16.6秒)。
  ベートーヴェン「歓喜の歌」(PD)を **ch1 ピアノ(旋律+和音) / ch2 ベース / ch3 ギター /
  ch10 ドラム**(GM の打楽器チャンネル)に編曲。編曲は自作なのでリポジトリと同じ MIT
- **実測(TD)**: 16.6秒・993サンプル。プログラムチェンジ ch1=1(Piano) / ch2=34(Finger Bass) /
  ch3=28(Clean Guitar)、**ch10 はプログラム無し**(GM の打楽器チャンネルなので不要)。
  ドラムは 36 キック / 38 スネア / 42 ハイハット / 49 クラッシュ。notes_held 7 / peak 0.21
...
### 2026-08-19 プラグイン更新後は必ず TD を再起動する(運用ルール)

- ユーザー指示「plugin を更新したら td を再起動して」。**ビルド → 常設インストール → その場で再起動**を
  自分で行い、**反映を確認**するところまでを1セットにする(パラメータや Info CHOP のチャンネルが
  増えているかで判定できる)
- **今回やらかしたこと**: `bank_status` を足したビルドを scratchpad へステージしただけで
  **常設へコピーし忘れ**、その状態で「反映済み」のつもりで進めた。ユーザーが先に気づいて再起動した。
  インストール後は `strings <installed>/Contents/MacOS/<exe> | grep <新しいシンボル>` で
  入っているかを確かめる(デバッグ用の取り残しが無いかも同時に見られる)
- 検証中にバージョン付きパス(/tmp)で読ませるのは引き続き有効だが、**それで終わらせず最後は
  常設へ入れて再起動**する。memory: [[restart-td-after-plugin-install]]

### 2026-08-19 AU Instrument に Sync to TD Tempo を追加

- ユーザー「TD の tempo と sync するトグルを追加して」。それまでは**ファイルのテンポマップ専従**で、
  TD の BPM は一切見ていなかった(`FF 51 03` を辿って tick→秒に展開し、位置は秒で進める設計)
- `Sync to TD Tempo` トグル: On で速度に **`root.time.tempo ÷ ファイルの BPM`** を掛ける。
  ファイル先頭のテンポを `MidiSeq::bpm` に記録して分母に使い、Info CHOP に `file_bpm` として出す
- **`OP_TimeInfo` にテンポは無い**(absFrame / frame / rate / rootFrame / rootRate / deltaFrames /
  deltaMS だけ)。**埋め込み Python で `root.time.tempo` を読む**しかない。毎cook は重いので
  **30 cook キャッシュ**(テンポはめったに変わらないので0.5秒の遅れは問題にならない)
- **実測**: 116 BPM のファイル + TD 300 BPM で、On のとき **2.586倍**(= 300/116。24.87秒の経過で
  64.33秒ぶん進み、3ループ+14.68秒が観測値と厳密一致)、Off でちょうど **1.00倍**
- **測定の教訓(再発)**: 最初ユーザーのデモノードで測ったら**位置が逆行**した(-47.9秒)。
  ユーザーが同時に同じノードを操作してファイルを差し替えていたため。
  **時間をまたぐ測定は自分の検証ノードで行う**([[toe-changes-are-users]] の裏返し)

### 2026-08-19 CoreMIDI In にも MIDI ファイル再生を入れた(パーサを共通ヘッダへ)

- ユーザー「CoreMIDI In にも同じ MIDI file 機能を入れたい」。**以前は「筋が悪い」と答えたが、
  状況が変わったので採用した**: TD 標準の MIDI In CHOP もファイルは読めるが、
  **タイムライン長のクリップを渡すだけでトランスポートが無い**。そこがこの op の埋める差になった
- **`MidiSeq` を `common/MidiSeq.h` へ切り出して AU Instrument と共有**。ObjC を使わない純 C++ なので
  ヘッダに実装を置いても別バンドルなら重複シンボルにならない(Multipeer In/Out と同じ型)
- CoreMIDI In 側の実装の要点: **イベントを `onVoiceLocked()` に流す**ので、
  **デバイスから来たときとまったく同じチャンネル**(`ch1n60` 等)に出る。
  ファイル再生と鍵盤演奏を重ねられる。`onVoice` を「ロックを取る版/取らない版」に分けた
  (受信スレッドは前者、ファイル再生はまとめてロックして後者)
- **プログラムチェンジの命名が2系統ある**ことが判明したので AU Instrument が両方読むようにした:
  - `ch<n>p` … TouchDesigner 標準の MIDI In CHOP。GM 慣習の **1始まり**
  - `ch<n>prog` … このリポジトリの CoreMIDI In CHOP。**生の MIDI 値**
  名前が違うので曖昧さなく判定できる。CoreMIDI In の `Program Change` トグルは**既定オフ**
- **実測(M2)**: 4パートのファイルで `ch1n* ch2n* ch3n* ch10n*` と `ch1prog 0 / ch2prog 33 /
  ch3prog 27` が出て、そのまま AU Instrument に繋ぐと正しい楽器で鳴った
  (`programs_sent` 3・peak 0.087・両 op とも警告なし)
- ついでに: **ファイル再生中はデバイス未選択の警告を出さない**ようにした(紛らわしいため)
- **踏んだミス**: `build.sh` の `rm -rf build` を置換したら、**コメント中の同じ文字列**に当たって
  スクリプトを壊した(バッククォートが不一致になり zsh が `unmatched \`` で停止)。
  **置換は最初の出現に当たる** — アンカーは一意な文字列にするか、置換後に必ず読み返す

### 2026-08-19 CoreMIDI In のファイル再生で連打が消える問題を修正

- ユーザー「CoreMIDI の MIDI File 再生だと AU Instrument で再生するのと違う。音が少ない」
- **原因: CHOP のゲートは1フレームに1回しか反転できない**。出力は1フレーム1サンプルなので、
  16.7ms 以内に off→on が起きるとゲートが 0 に戻らず、**再発音が消える**。
  自作素材のノートオフ隙間は6ティック(≈6.5ms)なので該当した
- **実測で原因を数値的に確定**: 8分のハイハット `ch10n42` は本来 64回/ループなのに **43回**。
  隙間 6.5ms ÷ フレーム 16.7ms = **39%** しか拾えない計算とぴったり一致した
  (立ち上がりを Execute DAT で数え、ファイル側の真値は Python で SMF をパースして比較)
- **修正: 同じノートを1回の cook で2回反転させない**。ぶつかったらそこで**イベント窓を切り**、
  続きは次のフレームへ回す。これで全打点が戻った(1ループ 64 に対し 68。残差は境界と位相)
- **踏んだオフバイワン**: 窓を `stop = e.t` で切ると、次の窓の `e.t <= a` に引っかかって
...
### 2026-08-21 MIDI ファイル再生の連続バグ4件を修正(CoreMIDI In / AU Instrument)

ユーザーの実機報告を追って、独立した4件を順に潰した。**どれも実測で原因を特定してから直した**。

1. **停止しても最後の音が鳴り続ける** — 「停止したら鳴っている音を止める」とREADMEに書いたのに
   実装が抜けていた。`mySeqPlaying` を持たせ、再生→停止の遷移で全ノートオフ。
   あわせて `Play` を全モードで有効にした(従来は Sequential でしか見ていなかった)
2. **モーツァルトだと最初の音が鳴り続ける(再生が止まる)** — 連打対策の「窓切り」を**時刻**で
   管理していたため、**同一ティックに同じ音の off と on がある譜面**で切り位置が進まず停止した。
   実ファイルを解析したら **2887箇所**該当。→ **イベント番号(索引)で管理**に変更。
   索引なら必ず1つ以上進むので停止しない
3. **頭の音が鳴らない / 音色が変** — `seekIndex` は「その時刻より後」を返すので、
   頭出し位置に `pos` をそのまま渡すと **tick 0 のイベントを丸ごと落とす**。
   モーツァルトは音色指定が全部 tick 0 にあるので、7パートの音色が全部落ちていた。
   `pos - 1e-9` を渡すように
4. **ループの2巡目以降が鳴らない** — 索引を取り直す行が seek 処理の**後ろ**にあり、
   折り返しで `mySeqPrev` を負にした直後にその行が走って**古い位置(ファイル末尾)の索引で
   上書き**していた。→ 取り直しを pos 計算より**前**に移動。
   あわせて折り返しは**余りではなく端ちょうど**へ戻す(余りを残すと 0〜pos の頭の音を飛ばす)

- ほかに、窓を切ったときに**時刻まで巻き戻して残り時間を捨てていた**のを修正(実測 0.95倍 → 1.00倍)。
  切るときは索引だけ残し、時刻は進める
- CoreMIDI In の Info CHOP に `file_position` / `file_duration` / `file_bpm` / `file_events` を追加
- **実測(修正後)**: CoreMIDI In で1ループ 230/222/222 ノート(真値223)、**頭のクラッシュが
  初回+3ループの4回とも鳴る**。AU Instrument も 231/223/223。
  モーツァルトは7パートの音色が全部渡り、再生も 14.8音/秒(ファイルの密度と一致)
...
### 2026-08-21 fps が落ちると音が乱れる件の調査(自作opで解決できるか)

- ユーザー「Audio File In + Audio Device Out で fps が 60 を切ると音が綺麗に再生されないことがある。
  自作 op で解決できないか」
- **実測した仕組み**: TD の音声 CHOP は timeslice で、`numSamples = レート × フレーム時間`。
  22050Hz のファイルで通常 367/368 サンプル(=16.7ms)、**重いフレームでは 1103 サンプル(=50ms)**を
  一度に出していた。つまり**TD は実時間を追っており、音がずれて溜まることはない**
- **したがって不具合はバッファの枯渇**: ハングしている間は**音が1サンプルも作られない**ので、
  Audio Device Out の待ち行列が減り続ける。**1回のハングが Buffer Length(既定0.15秒)を超えると
  デバイスが枯渇してプツッと切れる**
- **自作の「出力 op」では直らない**。音源側(Audio File In)も cook 駆動なので、
  ハング中は同じように何も作らない。バッファを大きくするのと同じことにしかならない
- **直るとしたら「ファイル読みを cook から切り離す」op**: 自前スレッドでファイルを読み、
  CoreAudio の IOProc へ直接流す。cook が何秒止まっても音は途切れない。
  実現性は確認済み(CA Process Tap で IOProc は既に使っている。デバイスのレート変更・hog mode も
  権限なしで通ることを実測済み)。代償は**音が TD の CHOP チェーンを通らない**こと
- **いますぐの緩和策**: `Buffer Length` を 0.3〜0.5 秒に上げる(そのぶん遅延する)。`Adjust Speed` もある
- **測定の注意**: `project.cookRate` は**目標fpsであって実測fpsではない**。負荷をかけても 60.0 のまま
  なので、これで「fps が落ちた」判定をしてはいけない。**フレーム時間は `numSamples ÷ レート`で見る**
  のが確実だった。なお 7680×4320 のノイズ+ブラーでも M2 では 60fps を維持し、
  持続的な低 fps は再現できなかった(ハングは1回 50ms のみ)

### 2026-08-21 CoreAudio Out CHOP 実装(ファイルプレイヤー統合・cook が止まっても音が続く)

- ユーザー「CoreAudio Out CHOP に AudioFile Player の機能を入れてしまうのはどう?」→ 採用。
  op の責任を「デバイスへ音を出す」に置き、音源を **入力0(CHOP)/ File Player / 両方ミックス**
  にする形(AU Instrument でノート入力とファイル再生を同居させたのと同じ型)
- **構造**: ExtAudioFile を自前スレッドでデコード(デバイスレートへ変換・リング約5秒)→
  `AudioDeviceCreateIOProcIDWithBlock` の IOProc が直接読む。入力0は cook がリングに積み、
  IOProc がミックス。CHOP 出力は実際に鳴った音のモニタコピー。
  hog mode(排他)トグル付き。IOProc 内はロック・確保なし(atomic インデックスのみ)
- **決定的な実測**: TD のメインスレッドを `time.sleep(2.0)` で完全に止めても、
  **ファイル再生は途切れず位置が進み続けた**(file_position 進行・先読み 131862 サンプル ≈ 2.7秒を維持)。
  同じ2秒停止を従来構成(Audio File In + Audio Device Out)にかけて比較(ユーザーの耳で確認)。
  = fps 低下で音が乱れる問題の、ファイル再生についての解
- **限界も明記**: 入力0(CHOP 経由の音)は従来どおり cook 駆動なので、TD が止まればその成分は止まる。
  自作 op で直せるのはファイル再生の部分だけ
- 検証手順のメモ: **音を出す検証は Filegain=0 で組み立ててから、鳴らす瞬間だけ上げる**
...
### 2026-08-22 AMX の有効活用先を実測(効く条件は「読んだバイトを何回使うか」)

- 前エントリの続き。ユーザー「AMX の他の有効活用方法は」→ 用途別に実測(`scratchpad/amxuses.c` /
  `amx2.c`)。**先に TD 標準の穴を確認**: CHOP に `Least Squares` / `Solve` / `Homography` /
  `Cluster` / `K-Means` / `Ambisonic` / `DTW` は**無い**(`Matrix CHOP` はあるが `Transform Order` を
  持つ 4x4 変換行列用)。SOP にも `Register` は無い
- **効く条件が実測ではっきりした ― 算術強度(読んだバイトを何回使うか)**:

  | ケース | 結果 |
  |---|---|
  | 100万点 × 4x4 変換 | **BLAS の方が3倍遅い**(素直 2.68ms / BLAS 10.0ms) |
  | 10万候補と類似度・クエリ1件(sgemv) | 13.3 ms(メモリ律速) |
  | 同じデータ・**クエリ64件まとめて**(sgemm) | 1件あたり **0.363 ms** = **37倍の効率** |

  **「大量の要素に小さい変換」はメモリ律速で AMX が効かない**(vDSP の要素演算が 0.9倍だったのと同根)。
  効くのは**両方の次元が大きい行列積・解く系・バッチ化した検索**
- **測定の罠(再発)**: LAPACK/BLAS は**初回呼び出しが極端に遅い**(制御点200 が 8.6ms → ウォームアップ後
  0.127ms、PCA 17ch が 3.0ms → 0.055ms)。**必ずウォームアップしてから最小値を取る**。
  最初これで「小さい方が遅い」という意味不明な表を出しかけた

**実測した有効用途(M2・60fps 予算 16.67ms)**

| 用途 | 規模 | 時間 |
|---|---|---|
| **PCA(共分散→固有値分解)** | 17ch × 6000フレーム | 0.055 ms |
| | **68ch × 6000フレーム**(Vision Pose 全関節 × 100秒) | **0.348 ms**(2%) |
| | 136ch × 6000フレーム | 1.035 ms |
| **連立方程式(RBF/TPS ワープ)** | 制御点 200 / 800 / 1600 / 3200 | 0.127 / 2.0 / 10.6 / 78 ms |
| **埋め込み検索**(512次元) | 10万候補・1クエリ / 64クエリ | 13.3 ms / 0.363 ms per query |

**op 候補(優先順・すべて CHOP/DAT/SOP なので TOP のような CPU 往復ペナルティが無い)**

1. **PCA / モーション解析 CHOP** ← 推し。Vision Pose の 68ch を数個の数字に圧縮して演出を駆動、
   姿勢クラスタリング、異常検出。**Vision 系の既存 op と直結する**
2. **RBF / TPS ワープ**(SOP or TOP): 散在制御点から滑らかな変形場。プロジェクションマッピングの
   微調整(Corner Pin の4点より自由)、モーフィング、実測点からの歪み補正。
   **制御点が動いたときだけ解けばよい**ので 800点までは実質無料
3. **埋め込み検索 DAT/CHOP**: MobileCLIP 512次元で数万規模の素材ライブラリ検索。バッチ化が鍵
4. **ホモグラフィ / カメラ姿勢 CHOP**: 4点対応から射影変換を解く。**TD の Corner Pin は変換を
   適用するだけで行列を出さない**ので、VisionRect の四隅から平面の向きを取り出したいときに要る
5. **マトリクスミキサー / アンビソニックス回転 CHOP**: N入力×M出力。TD に Ambisonic 系は無い
...
### 2026-08-22 GameController デモ: VJ とカメラのボタン割り当てを再編

- ユーザー指示で以下へ変更(パッドのボタンがこれで全部埋まった):
  ...
  | PALETTE | X | **D-PAD 左** |
  | STROBE | Y | **D-PAD 右** |
  | **FLY 前進 / 後進** | — | **A / B**(新規・DroneCamera 側) |
  | RESHUFFLE CITY | D-PAD 左右 | **キーボード 9 のみ**(パッドに空きが無くなった) |

- **A / B は VJ ではなく DroneCamera(コンポーネント)に入れた**。スティック前後と同じ
  「カメラが向いている方向」へ動くデジタル入力で、`mz` に加算してから **±1 に丸める**
  (スティックと同時に入れても速くならない)。再利用可能な部品なので tox 側にも入る
- キーボードは従来どおり(数字キーの割当は変えていない。1=ワイヤー 2=音連動 3=パレット
  4=ストロボ 5/6=強さ 7/8=本数 9=作り直し)
- **実測(一時コピー + 疑似パッド)**: A → z 34→-170(前進・Range 上限まで)、
  B → z 34→170(後進)。Hometilt を 0 にして水平飛行で比較し、y は 12 のまま動かないことも確認
- **途中で一度 A/B を「街の作り直し」に割り当てたが、ユーザーの続報でカメラの前後だと判明して
  差し戻した。** 指示が分割して届くときは、確定した分だけ進めて残りは待つ方が手戻りが少ない
- 揃えた箇所: `vj` DAT / `DroneCamera/fly` DAT とヘッダ / 画面の CONTROLS(`help` TOP)/
  `note` DAT / `palette/README.md` / `palette/DroneCamera.tox`

### 2026-08-22 DroneCamera: R STICK Y を上昇 / 下降へ(前後は A / B に一本化)

- ユーザー指示「R STICK の Y を CLIMB / DESCEND に変更して」
- **前後(視線方向)は A / B だけになった**。R STICK Y は上昇 / 下降で、**R2 / L2 も残す**
  (機能を減らさないため)。両方を**合算してから ±1 に丸める**ので同時に入れても速くならない
- 入力ブロックを `dz()`(デッドゾーン)ヘルパで書き直して見通しを良くした。
  キーボードは従来どおり(W S 前後 / A D 平行移動 / Q E 旋回 / Space Shift 昇降 / Z X FOV)
- **実測(一時コピー + 疑似パッド)**: R STICK Y = +1 → 高度 12→134.98(z は 34 のまま)、
  -1 → 134.98→0.70(Altmin)。A → z 34→-170 で高度は 12 のまま。R2 → 12→200.77 で併存を確認

**現在のパッド割り当て(全ボタン埋まっている)**

| 入力 | 機能 |
|---|---|
| A / B | 前進 / 後進(視線方向) |
| R STICK X | 水平の平行移動 |
| R STICK Y ・ R2 / L2 | 上昇 / 下降 |
| L STICK X | バンク旋回 |
| L STICK Y | チルト |
| L1 / R1 | FOV 望遠 / 広角 |
| X / Y | ワイヤーフレーム / オーディオ連動 |
| D-PAD 左 / 右 | パレット / ストロボ |
| D-PAD 上下 | 連動の強さ |
| Options / Menu | ビュー切替 / リセット |

街の作り直し(9)とビルの本数(7/8)は**キーボードのみ**

### 2026-08-26 Vision Subject にインスタンス選択と ID Map モードを追加

- ユーザー「Vision Subject にパラメータを足せない?」→ API を調べたところ、
  **`VNGenerateForegroundInstanceMaskRequest` 自体は `results` プロパティしか持たない**
  (精度・感度の調整ノブは存在しない)。使える余地は全部 `VNInstanceMaskObservation` 側にあり、
  そのうち2つが未使用だったので実装した
- **① インスタンス選択(`Instance` パラメータ)**: 生成メソッド3つはどれも `NSIndexSet` を取るのに、
  実装は**常に `obs.allInstances`(全被写体をまとめる)**を渡していた。0 = 全部 / 1..N = N番目だけ。
  Soft Mask と Cutout に効く(Instance / ID Map モードは常に全部)
  - **番号の振り方に依存しない実装**にした。`allInstances` の値ではなく
    `enumerateIndexesUsingBlock` で**順番の N 番目**を取るので、将来 1 始まりでなくなっても動く
  - 範囲外は `sel.count == 0` で検出して黒フレーム(**空の IndexSet を渡すとマスクが返ってしまう**
    ので、呼ぶ前に止める必要がある・実測で確認)
- **② `Instance ID Map` モード**: `obs.instanceMask`(ラベル画像)を **Mono32Float に生の番号のまま**
  出す。0 = 背景 / 1..N。既存の `Instance Masks` モードは RGBA の4チャンネルに詰めるので**4個が上限**
  だが、こちらは上限なし。正規化しないので TD 側で `== 2` のような比較がそのまま書ける
- **ついでに直した**: 被写体が居ないときの黒フレームが `Mono8Fixed` 固定で、
  通常時の `Mono32Float` と**フレームによって出力フォーマットが変わっていた**。
  `emitEmpty()` に集約してモード本来のフォーマットに合わせた

**単体ハーネスでの実測(`scratchpad/vstest.m`・TD を使わず Vision を直接叩く)**

| 素材 | allInstances | 全部 | 1番目 | 2番目 |
|---|---|---|---|---|
| sample_objects | {1, 2} | 57.821 % | 33.371 % | 24.451 % |
| sample_animals | {1, 2} | 22.731 % | 15.654 % | 7.077 % |
| sample_cutout | {1} | 10.582 % | 10.582 % | — |

**部分の和が全体に一致**(33.371+24.451=57.822)= マスクは重ならず、選択が正しく効いている。
`instanceMask` は **512x512 の OneComponent8** で、画素値は実際に 0 / 1 / 2(既存コードの
8bit 前提も正しいことを確認)

- **推測せず実測したのが効いた点**: `allInstances` が 1 始まりであること、`instanceMask` の
...
### 2026-09-22 CoreAudio Out README に Audio Device Out との違いの表を追加

- ユーザー指示。英日とも「Why this exists / 何のためにあるか」の直後に比較表を新設:
  音源(CHOP のみ / CHOP + 内蔵ファイルプレイヤー)、TD 停止時の挙動、レイテンシ
  (Buffer Length 0.15 秒 / デバイスバッファ約 14ms)、チャンネル(多ch + スピーカー配置 /
  **ステレオのみ**)、デバイスレート・バッファ・排他の制御可否、抜線時の挙動、出力の有無。
  最後に「どちらを使うか」の指針を1段落
- Audio Device Out の抜線時の挙動は**実測していない**ので「未比較」と書いた(推測を表に入れない)
