...
extern "C"{DLLEXPORT void FillDATPluginInfo(DAT_PluginInfo*i){if(!i->setAPIVersion(DATCPlusPlusAPIVersion))return;i->customOPInfo.opType->setString("Visionbarcode");i->customOPInfo.opLabel->setString("Vision Barcode");i->customOPInfo.opIcon->setString("VBC");i->customOPInfo.authorName->setString("SYGNAL Inc.");i->customOPInfo.minInputs=0;i->customOPInfo.maxInputs=0;}DLLEXPORT DAT_CPlusPlusBase*CreateDATInstance(const OP_NodeInfo*i){return new VisionBarcodeDAT(i);}DLLEXPORT void DestroyDATInstance(DAT_CPlusPlusBase*i){delete static_cast<VisionBarcodeDAT*>(i);}}
