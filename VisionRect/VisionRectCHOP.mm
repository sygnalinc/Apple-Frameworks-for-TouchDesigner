...
public:
 ...
 void execute(CHOP_Output*out,const OP_Inputs*in,void*)override{myExec++;bool active=in->getParInt("Active")!=0,flip=in->getParInt("Flip")!=0;Settings s;s.max=myMax;s.minAspect=std::clamp((float)in->getParDouble("Minaspect"),0.f,1.f);s.maxAspect=std::clamp((float)in->getParDouble("Maxaspect"),s.minAspect,1.f);s.minSize=std::clamp((float)in->getParDouble("Minsize"),0.f,1.f);s.minConfidence=std::clamp((float)in->getParDouble("Minconfidence"),0.f,1.f);s.tolerance=std::clamp((float)in->getParDouble("Tolerance"),0.f,45.f);
  ...
  std::vector<Rect>rows;{std::lock_guard<std::mutex>l(myMutex);rows=myRows;}for(int i=0;i<myMax*14;i++)out->channels[i][0]=0;const tdaspect::Mapper map{in->getParInt("Aspectcorrectuv")!=0,top?(float)top->textureDesc.width:0.f,top?(float)top->textureDesc.height:0.f};for(int r=0;r<myMax&&active&&r<(int)rows.size();r++){int b=r*14;out->channels[b][0]=1;out->channels[b+1][0]=rows[r].confidence;out->channels[b+2][0]=map.x(rows[r].bbox[0]);out->channels[b+3][0]=map.y(rows[r].bbox[1]);out->channels[b+4][0]=map.dx(rows[r].bbox[2]);out->channels[b+5][0]=map.dy(rows[r].bbox[3]);for(int i=0;i<4;i++){out->channels[b+6+i*2][0]=map.x(rows[r].corners[i*2]);out->channels[b+7+i*2][0]=map.y(rows[r].corners[i*2+1]);}}}
 void setupParameters(OP_ParameterManager*m,void*)override{OP_StringParameter top("Top");top.label="TOP";top.page="Vision Rect";m->appendTOP(top);tdaspect::appendAspectCorrect<OP_ParameterManager,OP_NumericParameter>(m,"Vision Rect");OP_NumericParameter a("Active");a.label="Active";a.page="Vision Rect";a.defaultValues[0]=1;m->appendToggle(a);OP_NumericParameter n("Maxrects");n.label="Max Rectangles";n.page="Vision Rect";n.defaultValues[0]=10;n.minSliders[0]=1;n.maxSliders[0]=10;n.minValues[0]=1;n.maxValues[0]=100;n.clampMins[0]=n.clampMaxes[0]=true;m->appendInt(n);
