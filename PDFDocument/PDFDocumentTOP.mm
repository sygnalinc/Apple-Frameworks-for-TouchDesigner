...
class PDFDocumentTOP final : public TOP_CPlusPlusBase {
public:
    PDFDocumentTOP(const OP_NodeInfo*, TOP_Context* c) : myContext(c) { myThread=std::thread([this]{ worker(); }); }
    ~PDFDocumentTOP() override { { std::lock_guard<std::mutex> l(myMutex); myQuit=true; } myCond.notify_all(); if(myThread.joinable()) myThread.join(); }
...
DLLEXPORT void FillTOPPluginInfo(TOP_PluginInfo* i) {
    ...
    i->customOPInfo.opType->setString("Pdfdocument");
    i->customOPInfo.opLabel->setString("PDF Document");
...
DLLEXPORT TOP_CPlusPlusBase* CreateTOPInstance(const OP_NodeInfo* i, TOP_Context* c) { return new PDFDocumentTOP(i,c); }
DLLEXPORT void DestroyTOPInstance(TOP_CPlusPlusBase* i, TOP_Context*) { delete static_cast<PDFDocumentTOP*>(i); }
}
