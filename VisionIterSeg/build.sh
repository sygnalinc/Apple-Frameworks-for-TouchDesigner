#!/bin/zsh
# Vision IterSeg TOP のビルド → build/VisionIterSegTOP.plugin
...
NAME=VisionIterSegTOP
OUT="build/$NAME.plugin/Contents"
DYLIB="libVisionIterSegHelper_$(date +%s).dylib"
rm -rf build
mkdir -p "$OUT/MacOS" "$OUT/Frameworks"

# GenerateIterativeSegmentationRequest は macOS 27+(#available ガード済み)。target は macos15
swiftc -O -emit-library -module-name VisionIterSegHelper \
  -target arm64-apple-macos15.0 \
  VisionIterSegHelper.swift \
...
/usr/libexec/PlistBuddy \
  ...
  -c 'Add :CFBundleIdentifier string tokyo.sygnal.visioniterseg-top' \
