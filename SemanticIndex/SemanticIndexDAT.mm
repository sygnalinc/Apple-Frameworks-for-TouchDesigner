...
namespace {
class SemanticIndexDAT final : public DAT_CPlusPlusBase {
public:
    SemanticIndexDAT(const OP_NodeInfo*) {}
    ~SemanticIndexDAT() override { stop(); }
...
private:
    ...
    void startSearch(const std::string& q, const std::string& mode, int maxr) {
        @autoreleasepool {
            ...
            myQuery=mq;
            __block SemanticIndexDAT* self_=this; int cap=maxr;
...
DLLEXPORT void FillDATPluginInfo(DAT_PluginInfo* i) {
    ...
    i->customOPInfo.opType->setString("Semanticindex");
    i->customOPInfo.opLabel->setString("Semantic Index");
...
DLLEXPORT DAT_CPlusPlusBase* CreateDATInstance(const OP_NodeInfo* i) { return new SemanticIndexDAT(i); }
DLLEXPORT void DestroyDATInstance(DAT_CPlusPlusBase* i) { delete static_cast<SemanticIndexDAT*>(i); }
}
