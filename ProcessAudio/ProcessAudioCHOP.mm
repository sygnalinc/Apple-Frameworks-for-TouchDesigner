...
class ProcessAudioCHOP final : public CHOP_CPlusPlusBase {
public:
    ProcessAudioCHOP(const OP_NodeInfo*) { for(int c=0;c<2;c++) myRing[c].assign(kRing,0.f); }
    ~ProcessAudioCHOP() override { @autoreleasepool { teardown(); } }
...
private:
    ...
    void setup(const std::string& mode, int pid, bool excludeSelf) {
        ...
        @autoreleasepool {
            ...
            __block ProcessAudioCHOP* self_=this;
...
DLLEXPORT void FillCHOPPluginInfo(CHOP_PluginInfo* i) {
    ...
    i->customOPInfo.opType->setString("Processaudio");
    i->customOPInfo.opLabel->setString("Process Audio");
...
DLLEXPORT CHOP_CPlusPlusBase* CreateCHOPInstance(const OP_NodeInfo* i) { return new ProcessAudioCHOP(i); }
DLLEXPORT void DestroyCHOPInstance(CHOP_CPlusPlusBase* i) { delete static_cast<ProcessAudioCHOP*>(i); }
}
