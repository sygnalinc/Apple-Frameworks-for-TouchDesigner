...
| プラグイン | 種類 | 内容 |
|---|---|---|
| [CoreMIDI Out](CoreMIDI/) | CHOP | **抜き差しに強い MIDI 送信。** デバイスを **UniqueID** で保持し、一覧はホットプラグで更新される(TD標準は起動時のスナップショット)。テスト送信はパラメータだけで完結 |
| [CoreMIDI In](CoreMIDI/) | CHOP | **DAW からテンポとタイムコードを受ける。** MIDI Clock を `bpm` / `beat` にし、MTC を解読する(TD はどちらも持たない)。UniqueID とホットプラグの扱いは Out と同じ |
