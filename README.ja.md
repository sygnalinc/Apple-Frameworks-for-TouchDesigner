...
| [Cinematic Video](Cinematic/) | TOP | **iPhone Cinematic動画**(macOS 26+): 深度(視差)マップ/**f値・ピント差し替え再レンダ**。**Movie File In と同じく自動再生**(Play Mode: Sequential / Locked to Timeline / Specify Index、Speed/Loop/Cue)。`Mode = All` で**再レンダ・原版の色・深度を3色バッファ同時出力**(`Color + Depth` は軽い2バッファ版)。メタデータ(フォーカス深度・被写体)は**Info CHOP**で出力 |
