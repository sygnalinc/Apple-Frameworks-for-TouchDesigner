...
| [Cinematic Video](Cinematic/) | TOP | **iPhone Cinematic動画**(macOS 26+): 深度(視差)マップ/**f値・ピント差し替え再レンダ**。**Movie File In と同じく自動再生**(Play/Speed/Loop/Cue)。`Mode = Both` で**色と深度を2色バッファ同時出力**。メタデータ(フォーカス深度・被写体)は**Info CHOP**で出力 |
