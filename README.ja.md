...
| [CreateML Training Recorder](CreateMLTrainingRecorder/) | CHOP | 実験中 |
...
| バージョン | 日付 | OP数 | 主な内容 |
|---|---|---|---|
| **[0.9.7](https://github.com/sygnalinc/Apple-Frameworks-for-TouchDesigner/releases/tag/v0.9.7)** | 2026-08-14 | 65 | **オペレータを5つ追加。** **MapKit MapView / LookAround / Search** — Apple マップを TOP でライブに: 3D地図を 57fps で飛び(`Markers DAT` で緯度経度を画面 u/v へ射影)、Look Around の街並み実写は視線が双方向、検索 / ジオコーディング / 経路 / カバレッジは DAT。**CoreMIDI Out / In** — ホットプラグと機材識別に対応した MIDI、DAW のトランスポート(MMC)と同期(MIDI Clock / MTC)、届いた鍵盤・パッド・ノブから自動でチャンネルを作る入力。**CoreImage RAW** は現像の既定が**撮影時のホワイトバランス**になり sRGB 出力に(以前は固定 6500K で上書きしていた)。ファイルの設定はパラメータへ流し込まれる。**Game Controller** は TD が最前面でなくても入力を受け取れるようになり、Info DAT と角速度チャンネルを追加。**RealityKit Capture** は点群を 3DGS 形式の PLY で書き出せる |
