...
extern "C"{DLLEXPORT void FillCHOPPluginInfo(CHOP_PluginInfo*i){if(!i->setAPIVersion(CHOPCPlusPlusAPIVersion))return;i->customOPInfo.opType->setString("Visiontrajectory");i->customOPInfo.opLabel->setString("Vision Trajectory");i->customOPInfo.opIcon->setString("VTJ");if(i->customOPInfo.opHelpURL)i->customOPInfo.opHelpURL->setString("https://github.com/sygnalinc/TDAppleOps/blob/main/VisionTrajectory/README.md");i->customOPInfo.authorName->setString("SYGNAL Inc.");
